...
for page in range(1, 156): #oyunun veritabanındaki belli sayıdaki tüm oyuncuları SQL tablosuna eklemek için bir döngü
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/129.0.0.0 Safari/537.36 Edg/129.0.0.0" #kendi user_agentinizi giriniz 
    }
    url = f"https://pesdb.net/efootball/?mode=max_level&all=1&page={page}" #temel sayfa yapısı
    ...
    if table:
        ...
        for row in rows:
            ...
            if len(satir) > 1:
                link = satir[1].find('a')
                if link:
                    oyuncu_url = "https://pesdb.net/efootball/" + link['href'] #o sayfanın içindeki oyuncu linki
                    oyuncu_urls.append(oyuncu_url)

        for oyuncu_url in oyuncu_urls:
            ...
            conn.commit()
            print(f"Veritabanına kaydedildi: {oyuncuAdi_deger} - Genel Derece: {genelDerece}")
            time.sleep(3.8) #scrap ederken bloklanmamak için 3.75 saniye ve üzerinde bir cooldown değeri girin.
cursor.close()